...
export async function getAllEventsWithStatus(userId) {
  if (isNeon) {
    return sql`
      SELECT e.*,
        CASE WHEN r.user_id IS NOT NULL THEN true ELSE false END AS is_registered
      FROM events e
      LEFT JOIN registrations r ON e.id = r.event_id AND r.user_id = ${userId}
      ORDER BY e.event_start_date ASC
    `;
  }
  notImplemented('getAllEventsWithStatus');
}
